...
class VideoProcessor {
    ...
    validateEnvironment() {
        const required = ['videoBucket', 'videoFileName', 'videoId', 'outputBucket'];
    ...
    async downloadVideo(outputPath) {
        try {
            ...
            console.log(`Video downloaded successfully: ${(fileSize / 1024 / 1024).toFixed(2)} MB`);
            return fileSize;
        }
        catch (error) {
    ...
    async sendWebhook(status, manifestUrl) {
        ...
        try {
            ...
            await axios_1.default.post(this.env.webhookUrl, payload, {
                timeout: 10000,
                headers: {
    ...
    async processVideoWithFFmpeg(inputPath, outputDir) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const ffmpegArgs = [
                ...
                '-t', this.env.maxProcessingTime.toString(),
                '-filter_complex',
                ...
                ...(this.env.processingPriority === 'low' ? [
                    ...
                    '-map', '[v1out]', '-c:v:0', 'libx264', '-b:v:0', '2500k', '-maxrate:0', '2750k', '-bufsize:0', '5000k',
                    '-map', '[v2out]', '-c:v:1', 'libx264', '-b:v:1', '1200k', '-maxrate:1', '1320k', '-bufsize:1', '2400k',
                    '-map', '[v3out]', '-c:v:2', 'libx264', '-b:v:2', '600k', '-maxrate:2', '660k', '-bufsize:2', '1200k',
                ] : [
                ...
                // Output manifest
                path_1.default.join(outputDir, 'manifest.mpd')
            ];
            ...
            ffmpeg.on('close', (code) => {
                if (code === 0) {
                    ...
                    console.log(`Processing mode: ${this.env.instanceType} (${this.env.ffmpegPreset} preset)`);
                    resolve();
                }
                else {
            ...
            setTimeout(() => {
                if (ffmpeg && !ffmpeg.killed) {
                    console.warn(`FFmpeg timeout reached (${this.env.maxProcessingTime}s), terminating process`);
                    ffmpeg.kill('SIGTERM');
    ...
    async uploadProcessedFiles(outputDir) {
        ...
        try {
            ...
            const uploadPromises = files.map(async (file) => {
                ...
                const contentType = file.endsWith('.mpd') ? 'application/dash+xml' :
                    file.endsWith('.m4s') ? 'video/mp4' : 'application/octet-stream';
                const key = `${this.env.videoId}/${file}`;
                console.log(`Uploading: ${key} (${(fileContent.length / 1024).toFixed(2)} KB)`);
                const command = new client_s3_1.PutObjectCommand({
                    ...
                    CacheControl: file.endsWith('.mpd') ? 'max-age=300' : 'max-age=31536000', // 5 min for manifest, 1 year for segments
                });
            ...
            console.log(`All files uploaded successfully. Total size: ${(totalSize / 1024 / 1024).toFixed(2)} MB`);
            return results;
        }
        catch (error) {
    ...
    async cleanup() {
        try {
            await fs_1.promises.rm(this.tempDir, { recursive: true, force: true });
    ...
    async process() {
        ...
        try {
            ...
            const inputPath = path_1.default.join(this.tempDir, 'input.mp4');
            const outputPath = path_1.default.join(this.tempDir, 'output');
            ...
            const estimatedCost = this.env.instanceType === 'spot' ? '$0.30' : '$1.00'; // Rough estimate
            console.log('='.repeat(50));
            ...
            console.log(`Estimated cost: ${estimatedCost} (${this.env.instanceType === 'spot' ? '70% savings' : 'standard rate'})`);
