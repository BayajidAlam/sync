...
router.post('/processing-complete', async (req: express.Request, res: express.Response): Promise<void> => {
  try {
    ...
    if (status === VideoStatus.READY && manifestUrl) {
      ...
      updatedVideo = await videoService.updateVideoStatus(videoId, status as VideoStatus)
    }
