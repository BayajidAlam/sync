...
const calculateEstimatedSavings = () => {
    ...
    if (config.S3_LIFECYCLE_ENABLED)
        savings += 85;
    // Fargate Spot savings
    if (config.ECS_USE_FARGATE_SPOT)
        savings += 108;
...
const getActiveOptimizations = () => {
    const optimizations = [];
    if (config.S3_LIFECYCLE_ENABLED)
        optimizations.push('S3 Lifecycle Policies');
    if (config.S3_INTELLIGENT_TIERING)
        optimizations.push('S3 Intelligent Tiering');
    if (config.ECS_USE_FARGATE_SPOT)
        optimizations.push('Fargate Spot Instances');
    if (config.CLOUDFRONT_PRICE_CLASS === 'PriceClass_100')
    ...
    if (!config.DETAILED_MONITORING)
        optimizations.push('Reduced Monitoring');
    if (config.LOG_RETENTION_DAYS <= 7)
        optimizations.push('Short Log Retention');
