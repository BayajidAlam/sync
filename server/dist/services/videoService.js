...
export class VideoService {
    ...
    async createVideo(videoId, request) {
        try {
    ...
    async updateVideoStatus(id, status) {
        try {
            const video = await VideoModel.findByIdAndUpdate(id, { status }, { new: true });
            if (!video)
                return null;
    ...
    async markVideoAsUploaded(videoId) {
        try {
            ...
            await sqsService.sendVideoProcessingMessage(config.S3_BUCKET_RAW, `videos/${videoId}/${video.filename}`, videoId);
            // Update to processing
            const processingVideo = await this.updateVideoStatus(videoId, VideoStatus.PROCESSING);
            ...
            const errorVideo = await this.updateVideoStatus(videoId, VideoStatus.ERROR);
    ...
    async updateVideo(id, updates) {
        try {
            const video = await VideoModel.findByIdAndUpdate(id, { $set: updates }, { new: true });
            if (!video) {
                return null;
            }
            return this.mapDocumentToVideo(video);
    ...
    async markVideoAsReady(videoId, manifestUrl) {
        try {
            const updates = { status: VideoStatus.READY };
            if (manifestUrl) {
                updates.manifestUrl = manifestUrl;
            }
            const video = await VideoModel.findByIdAndUpdate(videoId, { $set: updates }, { new: true });
            if (!video) {
                return null;
            }
            return this.mapDocumentToVideo(video);
